...
def add_feature_continuous_condition(df_X, cont_feature_name, indicator, number):
    ...
    if pd.isnull(df_X[cont_feature_name]).any():
        df_X[cont_feature_name + "_is_null"] = pd.isnull(df_X[cont_feature_name])
        df_X[cont_feature_name][pd.isnull(df_X[cont_feature_name])] = np.mean(df_X[cont_feature_name][~pd.isnull(df_X[cont_feature_name])])
...
#     df_X[cont_feature_name][~pd.isna(df_X[cont_feature_name])] = np.mean(df_X[cont_feature_name][~pd.isna(df_X[cont_feature_name])])
    df_X[cont_feature_name + "_" + str(indicator) + "_" + str(number)] = ops[indicator](df_X[cont_feature_name], number)
    df_X[cont_feature_name][ops[indicator](df_X[cont_feature_name],number)] = np.mean(df_X[cont_feature_name][~ops[indicator](df_X[cont_feature_name],number)])
    return df_X
...
def add_feature_continuous_null(df_X, cont_feature_name):
    ...
    if (df_X[cont_feature_name] == np.inf).any():
        df_X[cont_feature_name + "_was_inf"] = (df_X[cont_feature_name] == np.inf)
        df_X[cont_feature_name][df_X[cont_feature_name] == np.inf] = np.mean(df_X[cont_feature_name][df_X[cont_feature_name] != np.inf])

    if (df_X[cont_feature_name] == -np.inf).any():
        df_X[cont_feature_name + "_was_neg_inf"] = (df_X[cont_feature_name] == np.inf)
        df_X[cont_feature_name][df_X[cont_feature_name] == -np.inf] = np.mean(df_X[cont_feature_name][df_X[cont_feature_name] != -np.inf])

    if pd.isnull(df_X[cont_feature_name]).any():
        df_X[cont_feature_name + "_was_null"] = pd.isnull(df_X[cont_feature_name])
        df_X[cont_feature_name][pd.isnull(df_X[cont_feature_name])] = np.mean(df_X[cont_feature_name][~pd.isnull(df_X[cont_feature_name])])
    return df_X
